...
subplot(3,1,2);
makebar(s_depth, s_depth, l_depth);
ylabel('Mean Error (mm)');
title('Depth Predictions');

subplot(3,1,3);
makebar(s_temp, s_temp, l_temp);
...
%     errorbar([1.86 2.14], means(2,:), mneg, mpos, 'color', 'k', 'LineStyle', 'none', 'lineWidth', 2);
    errorbar([2.86 3.14], means(2,:), lneg, lpos, 'color', 'k', 'LineStyle', 'none', 'lineWidth', 2);
